...
class CycleGANModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        parser.add_argument('--lambda_self_distill', type=float, default=1e1,
                            help='weight for self distill')
        ...
        parser.set_defaults(norm='instance', dataset_mode='unaligned',
                            ...
                            nepochs=100, nepochs_decay=100, save_epoch_freq=20,project=None, name=None)
    ...
    def forward(self):
        ...
        self.fake_B1, self.fake_B = self.netG_A(self.real_A)  # G_A(A)
        self.rec_A1,self.rec_A = self.netG_B(self.fake_B)  # G_B(G_A(A))
        self.fake_A1,self.fake_A = self.netG_B(self.real_B)  # G_B(B)
        self.rec_B1,self.rec_B = self.netG_A(self.fake_A)  # G_A(G_B(B))
    ...
    def backward_G(self):
        ...
        # 自蒸馏添加
        lambda_self_distill = self.opt.lambda_self_distill

        # Identity loss
        if lambda_idt > 0:
            # G_A should be identity if real_B is fed: ||G_A(B) - B||
            self.idt_A1,self.idt_A2 = self.netG_A(self.real_B)
            self.loss_G_idt_A = self.criterionIdt(self.idt_A2, self.real_B) * lambda_B * lambda_idt
            # G_B should be identity if real_A is fed: ||G_B(A) - A||
            self.idt_B1,self.idt_B2 = self.netG_B(self.real_A)
            self.loss_G_idt_B = self.criterionIdt(self.idt_B2, self.real_A) * lambda_A * lambda_idt
        else:
            self.loss_G_idt_A = 0
            self.loss_G_idt_B = 0
        ...
        # 自蒸馏对应的损失函数
        # 可选择的损失函数
        # （OMGD文章里的）SSIM、feature、style
        ssim_loss = pytorch_ssim.SSIM()
        self.loss_G_SSIM = lambda_self_distill * (2 - ssim_loss(self.fake_B1, self.fake_B)-ssim_loss(self.fake_A1, self.fake_A))

        self.loss_G_self_distill = self.loss_G_SSIM

        # combined loss and calculate gradients
        self.loss_G = self.loss_G_A + self.loss_G_B + self.loss_G_cycle_A + self.loss_G_cycle_B + self.loss_G_idt_A + self.loss_G_idt_B + self.loss_G_self_distill
